...
<tr>
    ...
    <td>
        @if(!empty($line->product->category)) {{$line->product->category->name}} @endif
    </td>
    ...
    <td>
        @if(!empty($line->variation->color)) {{$line->variation->color->name}} @endif
    </td>

    <td>
        @if(!empty($line->variation->size)) {{$line->variation->size->name}} @endif
    </td>

    <td>
        @if(!empty($line->variation->grade)) {{$line->variation->grade->name}} @endif
    </td>
    ...
    @endphp
    <td>
        @if(isset($current_stock)){{@num_format($current_stock)}}@else{{0}}@endif
    </td>
    <td>{{$add_stock->supplier->name}}</td>
    <td style="width: 100px !important"><input type="text" class="form-control email"
            name="remove_stock_lines[{{$i}}][email]" id="" value="{{$add_stock->supplier->email}}"></td>
    <td>{{$add_stock->invoice_no}}</td>
    <td>{{@format_date($add_stock->transaction_date)}}</td>
    <td>@if(!empty($payment_status_array[$add_stock->payment_status])) <a
            data-href="{{action('TransactionPaymentController@show', $add_stock->id)}}" data-container=".view_modal"
            class="btn btn-modal">{{$payment_status_array[$add_stock->payment_status]}}</a>@endif
    </td>
    <td><input type="text" class="form-control notes" name="remove_stock_lines[{{$i}}][notes]" id="" value="">
    </td>
    <td>
        @if(isset($line->quantity)){{@num_format($line->quantity)}}@else{{0}}@endif
    </td>
    <td>
        ...
        <span class="error stock_error hide">@lang('lang.quantity_should_not_greater_than')
            @if(isset($line->quantity)){{@num_format($line->quantity)}}@endif</span>
        <input type="hidden" class="form-control sub_total" min=1 name="remove_stock_lines[{{$i}}][sub_total]" required
            value="0">
        <input type="hidden" class="form-control purchase_price" min=1 name="remove_stock_lines[{{$i}}][purchase_price]"
            required value="@if(isset($line->purchase_price)){{@num_format($line->purchase_price)}}@else{{0}}@endif">
    </td>
    ...
    @endcan
    <td>
        @if(isset($line->product->sell_price)){{@num_format($line->product->sell_price)}}@else{{0}}@endif
    </td>
